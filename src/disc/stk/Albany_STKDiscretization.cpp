...
void STKDiscretization::computeVectorSpaces()
{
  ...
  for (auto& it1 : tmp_map) {
    ...
    indices.resize(numNodes+numGhostedNodes);
    for (int i=0; i<numGhostedNodes; ++i) {
      // STK ids start from 1. Subtract 1 to get 0-based indexing.
      const GO nodeId = bulkData.identifier(nodes[i]) - 1;
      indices[numNodes+i] = nodeId;
    }
    ...
    for (auto& it2 : it1.second) {
      ...
      if (numComponents == 1) {
        ...
        dofs->overlap_vs = ov_part_node_vs;
        dofs->vs         = part_node_vs;
        ...
        dofs->dofManager.setup(numComponents,
                               dofs->vs_indexer->getNumLocalElements(),
                               dofs->vs_indexer->getMaxGlobalGID(),
                               interleavedOrdering);
