...
evaluateFields(typename PHALTraits::EvalData workset)
{
  ...
  Kokkos::parallel_for(this->getName(),RangePolicy(0,sideSet.size),
    ...
    const auto pos = sideSet.side_pos.view_device()(iside);
    const int numSideNodes = snc(pos);
    for (int node=0; node<numSideNodes; ++node) {
      double contrSol[3] = {0.0, 0.0, 0.0};
      for(int il=0; il<=nlayers; ++il) {
        for(int comp=0; comp<ncomps; ++comp)
          contrSol[comp] += x_data(localDOF.view_device()(iside, node, il, comp+l_offset))*w(il);
      }
      for (int comp=0; comp<ncomps; ++comp) {
      ...
      for(int il=0; il<=nlayers; ++il) {
        for(int comp=0; comp<ncomps; ++comp) {
          ...
          if (g_xx_is_active||g_px_is_active) {
            contrDir[comp] += direction_x_data(localDOF.view_device()(iside,node,il,comp+l_offset))*w(il);
          }
        }
      }
